...
from kivy.uix import label, popup, button
...
class HelpButton(Button):
    def __init__(self, text):
        ...
        hbox.padding = 10
        help_logo = Image()
        help_logo.source = "icons/seed.png"
        help_logo.width=20

        content_label = Label(text=text)
        content_label.padding_x = 5

        hbox.add_widget(help_logo)
        hbox.add_widget(content_label)

        ok_button = Button(text="OK")

        vbox.add_widget(hbox)
        vbox.add_widget(ok_button)

        m_popup = Popup(title="Help",
            content=vbox,
            size_hint=(None, None), size=(600, 600),
            auto_dismiss=False
        )

        ok_button.bind(on_press=m_popup.dismiss())

        help_button.bind(on_press=m_popup.dismiss())
...
def ok_cancel_buttons(dialog, cancle_callback, ok_callback, ok_label=_("OK") ):
    hbox = BoxLayout(orientation="horizontal")
    cancel_button = ToggleButton(text="Cancel")
    cancel_button.padding_x = 5
    cancel_button.bind(on_press=cancle_callback)
    hbox.add_widget(cancel_button)
    ok_button = ToggleButton(text=ok_label)
    ok_button.bind(on_press=ok_callback)
    hbox.add_widget(ok_button)
    return hbox
...
# refactoring code
def message_box(title, label, callback_function):
    m_layout = BoxLayout(orientation="vertical")
    m_layout.padding = 20

    m_label = Label(text=label)
    m_label.padding_y = 10
    m_layout.add_widget(m_label)

    m_button = Button(text="OK")
    m_button.padding_y = 10
    m_layout.add_widget(m_button)

    m_popup = Popup(title=title,
      content=m_layout,
      size_hint=(None, None), size=(600, 600),
      auto_dismiss=False
      )

    def on_ok_button_click(instance):
        callback_function()

    m_button.bind(on_press=on_ok_button_click)
...
class ElectrumWindow(QMainWindow):
    ...
    def show_seed(self, seed, imported_keys, parent=None):
        ...
        dialog.size_hint = 1,1

        def handle_btn_ok_click(instance):
            dialog.dismiss()
            parent.handle_show_seed_callback()

        brainwallet = ' '.join(mnemonic.mn_encode(seed))

        label1 = Label(text="Your wallet generation seed is"+ ":")

        seed_text = TextInput(text=brainwallet)
        seed_text.readonly = True
        seed_text.multiline = True
        #don't have option set maximum_height
        ...
        #don't have wordwrap option
        logo = Image()
        logo.source = "icons/seed.png"
        logo.width=20
        #don't found option setmaximum width, scaled.

        #haven't migrate qr code widget.
        #qrw = QRCodeWidget(seed)
        qrw = Label(text="Qr code")

        ok_button = Button(text='OK')
        ok_button.bind(on_press=handle_btn_ok_click)
        ok_button.halign = "right"
        ok_button.size_hint_x = .5
        ok_button.size_hint_y = .5
        #ok_button.padding = 10

        mainLayout = BoxLayout(orientation='vertical')
        mainLayout.padding = 20
        #logo, qrcode, seed section
        layout1 = BoxLayout(orientation='horizontal')
        #layout1.padding = 10

        #layout11 contains logo, seed
        layout11 = BoxLayout(orientation='vertical')


        layout111 = BoxLayout(orientation='horizontal')
        layout111.add_widget(logo)
        layout111.add_widget(label1)

        layout11.add_widget(layout111)

        print seed_text.text
        layout11.add_widget(seed_text)

        layout1.add_widget(layout11)
        layout1.add_widget(qrw)

        mainLayout.add_widget(layout1)

        mainLayout.add_widget(label2)
        mainLayout.add_widget(ok_button)
        dialog.add_widget(mainLayout)
        dialog.open()
...
#
#         d = ModalView(size_hint=(None, None))
#         d.size_hint = 1,1
#
#         vbox = BoxLayout(orientation="vertical")
#         vbox.padding = 20
...
#         hbox.padding = 10
#         l = Image()
#         l.source = "icons/network.png"
#         l.width=20
#         hbox.add_widget(l)
#         hbox.add_widget(Label(status))
#         vbox.add_widget(hbox)
#
#         grid = GridLayout(cols=2)
#         vbox.add_widget(grid)
#
#         server_protocol = ComboEdit()
#         server_host = TextInput()
#         server_port = TextInput()
#
#         protocol_names = ['TCP', 'HTTP', 'SSL', 'HTTPS']
#         protocol_letters = 'thsg'
#         list_protocol_item = []
#         for item in protocol_names:
#             label_item = Label(text = item)
#             list_protocol_item.append(label_item)
#
#         server_protocol.options = list_protocol_item
#
#         grid.add_widget(Label(text="Server:"))
#         hbox = BoxLayout(orientation="horizontal")
#         hbox.add_widget(server_protocol)
#         hbox.add_widget(server_host)
#         hbox.add_widget(server_port)
#         grid.add_widget(hbox)
#
...
#
#         server_protocol.drop_down.bind(on_select=change_protocol)
#
#         label = _('Active Servers') if wallet.interface.servers else _('Default Servers')
#
#         vbox = BoxLayout(orientation="vertical")
#         label = _('Active Servers') if wallet.interface.servers else _('Default Servers')
#         servers_list_header = Label(text = label)
#         servers_list_widget = BoxLayout(orientation="vertical")
#         vbox.add_widget(servers_list_header)
#         vbox.add_widget(servers_list_widget)
#
#         for _host in servers_list.keys():
#             pruning_level = servers_list[_host].get('pruning','')
#             item = BoxLayout(orientation="horizontal")
#             servers_list_widget.add_widget(item)
#             label_host = Label(_host)
#             item.add_widget(label_host)
#             label_pruning_level = Label(pruning_level)
#             item.add_widget(label_pruning_level)
#
#             if (not parent.expert_mode if parent else True):
#                 item.remove_widget(label_pruning_level)
#
...
#             server_protocol.text = protocol_names[protocol_letters.index(protocol)]
#
#             if not plist: return
...
#         """
#         #will add this handle
#         server_host.s
#         if not wallet.config.is_modifiable('server'):
#             #print enable, disable all component
#             print("enable, disable all component")
...
#         """
#
#         autocycle_cb_group = BoxLayout(orientation="horizontal")
#
#         autocycle_cb_label = Label(text="Try random servers if disconnected")
#         autocycle_cb_group.add_widget(autocycle_cb_label)
#
#         autocycle_cb = CheckBox() (_())
#         autocycle_cb.active = True
#
#         autocycle_cb_group.add_widget(autocycle_cb)
#         grid.add_widget(autocycle_cb_group)
#         if not wallet.config.is_modifiable('auto_cycle'): autocycle_cb.active = False
#
#
#         proxy_mode = ComboEdit()
#         proxy_host = TextInput()
#         proxy_host.width = 200
#         proxy_port = TextInput()
#         proxy_port.width = 60
#
#         list_protocol_mode_item = []
#         for item in ['NONE', 'SOCKS4', 'SOCKS5', 'HTTP']:
#             label_protocol_model_item = Label(text = item)
#             list_protocol_mode_item.append(label_protocol_model_item)
#
#         proxy_mode.options = list_protocol_mode_item
#
#         def check_for_disable(index = False):
#             if proxy_mode.text != 'NONE':
#                 print("run enable proxy_host, proxy_port")
#                 #proxy_host.setEnabled(True)
#                 #proxy_port.setEnabled(True)
#             else:
#                 print("run disable proxy_host, proxy_port")
...
#
#         proxy_config = interface.proxy if interface.proxy else { "mode":"none", "host":"localhost", "port":"8080"}
#         print("run proxy_mode.setCurrentIndex")
#         #proxy_mode.setCurrentIndex(proxy_mode.findText(str(proxy_config.get("mode").upper())))
#         proxy_host.setText(proxy_config.get("host"))
#         proxy_port.setText(proxy_config.get("port"))
#
#         grid.addWidget(Label('Proxy' + ':'))
#         grid.addWidget(proxy_mode)
#         grid.addWidget(proxy_host)
#         grid.addWidget(proxy_port)
#
#         #vbox.addLayout(ok_cancel_buttons(d))
#         def cancle_callback():
#             pass
...
#             pass
#
#         vbox.add_widget(ok_cancel_buttons(d, cancle_callback, ok_callback))
#         #d.setLayout(vbox)
#         d.add_widget(vbox)
#
#         """
...
#         interface.set_server(server, proxy)
#         wallet.config.set_key('auto_cycle', autocycle_cb.isChecked(), True)
#         self.parent.handle_network_dialog_callback(True)
        pass

    @staticmethod
    def network_dialog_custom(wallet, callback_function, parent=None):
        #handle gui network dialogue
        interface = wallet.interface
        if parent:
            if interface.is_connected:
        ...
        l.width=20
        hbox.add_widget(l)
        hbox.add_widget(Label(text=status))
        vbox.add_widget(hbox)
        ...
        vbox_server = BoxLayout(orientation="vertical")
        label = _('Active Servers') if wallet.interface.servers else _('Default Servers')
        servers_list_header = Label(text = label)
        servers_list_widget = BoxLayout(orientation="vertical")
        vbox_server.add_widget(servers_list_header)
        vbox_server.add_widget(servers_list_widget)

        for _host in servers_list.keys():
            pruning_level = servers_list[_host].get('pruning','')
            item = BoxLayout(orientation="horizontal")
            servers_list_widget.add_widget(item)
            label_host = Label(text=_host)
            item.add_widget(label_host)
            label_pruning_level = Label(text=pruning_level)
            item.add_widget(label_pruning_level)
        ...
        list_protocol_mode_item = []
        for item in ['NONE', 'SOCKS4', 'SOCKS5', 'HTTP']:
            temp_label = Label(text = item)
            list_protocol_mode_item.append(temp_label)
        ...
        print("run proxy_mode.setCurrentIndex")
        #proxy_mode.setCurrentIndex(proxy_mode.findText(str(proxy_config.get("mode").upper())))
        proxy_host.text = proxy_config.get("host")
        proxy_port.text = proxy_config.get("port")

        grid.add_widget(Label(text='Proxy' + ':'))
        grid.add_widget(proxy_mode)
        grid.add_widget(proxy_host)
        grid.add_widget(proxy_port)
        ...
        d.open()
        """
        server = unicode( server_host.text ) + ':' + unicode( server_port.text ) + ':' + (protocol_letters[server_protocol.text])
...
class ElectrumGui:
    ...
    def __init__(self, wallet, config, parent, app=None):
        self.wallet = wallet
        self.config = config
        self.parent = parent
    ...
    def restore_or_create(self):
        ...
        mainLayout = BoxLayout(orientation='vertical')
        lblMessage = Label(text=msg)
        buttonListLayout = BoxLayout(orientation='horizontal')
        btnCreate = Button(text='Create')
        btnRestore = Button(text='Restore')
        btnCancel = Button(text='Cancel')
        buttonListLayout.add_widget(btnCreate)
        buttonListLayout.add_widget(btnRestore)
        buttonListLayout.add_widget(btnCancel)
        mainLayout.add_widget(lblMessage)
        mainLayout.add_widget(buttonListLayout)
        ...
        def handle_btn_click(instance):
            self.popup.dismiss()
            self.parent.handle_restore_or_create_callback(instance)

        btnCreate.bind(on_press=handle_btn_click)
        btnRestore.bind(on_press=handle_btn_click)
        btnCancel.bind(on_press=handle_btn_click)

        self.popup.open()

    def on_verify_seed_error(self, r):
        print("on_verify_seed_error")
        print("r: ")
        print(r)
        print("self.wallet.seed: ")
        print(self.wallet.seed)
        if r != self.wallet.seed:
            print("case differentce")
            """
            QMessageBox.warning(None, _('Error'), 'incorrect seed', 'OK')
            return False
            """
            message_box("Error", "incorrect seed", self.parent.handle_on_seed_verify_fail)

        else:
            #return True
            self.parent.handle_on_seed_verify_successed()

    def verify_seed(self):
        self.seed_dialog(False)
    ...
    def seed_dialog(self, is_restore=True):
        ...
        seed_e = TextInput()
        seed_e.multiline = True
        vbox.add_widget(seed_e)

        if is_restore:
            hbox = BoxLayout(orientation="horizontal")
            gap_limit_label = Label(text="Gap limit")
            gap_limit_label.padding_x = 5
            ...
            #help_button = Button(text="?")
            help_button = HelpButton(text="?")

            hbox.add_widget(gap_limit_label)
            hbox.add_widget(gap_e)
            hbox.add_widget(help_button)

        def on_cancel_click(instance):
            self.on_verify_seed_error("")
        ...
        def on_ok_click(instance):
            try:
                ...
                print("case exception 1")
                try:
                    seed = mnemonic.mn_decode( seed.split(' ') )
                except:
                    message_box('Error', 'I cannot decode this', None)
                    self.on_verify_seed_error("")

                if not seed:
                    message_box('Error', 'No seed', None)
                    self.on_verify_seed_error("")

            if not is_restore:
                self.on_verify_seed_error(seed)
            else:
                try:
                    gap = int(unicode(gap_e.text()))
                except:
                    #QMessageBox.warning(None, _('Error'), 'error', 'OK')
                    message_box("Error", "error", None)
                    return
                return seed, gap

        ok_cancel_button = ok_cancel_buttons(d, on_cancel_click, on_ok_click)
        vbox.add_widget(ok_cancel_button)
        d.add_widget(vbox)
        d.open()
    ...
    def network_dialog_custom(self, callback_function):
        return ElectrumWindow.network_dialog_custom( self.wallet, callback_function, parent=None )

    def show_seed(self):
        ElectrumWindow.show_seed(self.wallet.seed, self.wallet.imported_keys)

    def show_seed_with_parent(self, parent):
        ElectrumWindow.show_seed(self.wallet.seed, self.wallet.imported_keys, parent)
